  ...
  <el-main>
    <el-card>
      ...
      <el-popover
         ...
         <div style="text-align: right; margin: 0">
           ...
           <el-button type="primary" size="mini" @click="deleteRoles">确定</el-button>
         </div>
        <el-button class="btn" slot="reference" icon="el-icon-delete" type='danger'>删除</el-button>
      </el-popover>
      <!-- 添加按钮 -->
    ...
    element-loading-background="rgba(0, 0, 0, 0.8)" :header-cell-style="{
      ...
      @selection-change="handleSelectionChange">
        <el-table-column  :selectable='cannotchoosed'  type='selection'></el-table-column>
        <el-table-column   prop='_id' class-name='table_col' sortable label='ID'></el-table-column>
        <el-table-column  prop='roleName' label='角色名'></el-table-column>
        <el-table-column  prop='powers' label='拥有权限'></el-table-column>
        <el-table-column  prop='describe' label='具体描述'></el-table-column>
        <el-table-column  label='操作'>
          <template slot-scope="scope" >
            <el-button  :disabled="scope.row.powers[0] ==='*'" type='primary' icon='el-icon-edit' @click='editDialog(scope.row)' >编辑</el-button>
            <el-button @click='removeBox(scope.row._id)' :disabled="scope.row.powers[0] ==='*'" type='danger' icon='el-icon-delete'>删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
<script>
import {addRole,editRole, getRole, deleteRole, deleteRoles} from '@/api/global'
...
export default {
  ...
  methods: {
   ...
   async init(obj){
     this.loading = true
   const {results:{count,data}}= await getRole(obj)
